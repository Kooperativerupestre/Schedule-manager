...
class WorkstationService:
    @staticmethod
    async def add(
        person_id: UUID, request: WorkstationAddRequest, conn: AsyncConnection[DictRow]
    ) -> UUID:
        await UnitValidator.validate_workstation_lifecycle_capability(
            person_id, request.unit_id, conn
        )
        response = await WorkstationRepository.add(
        ...
        await CapabilitiesRepository.add(
            person_id,
            response.id,
            CapabilityInput(Resource.WORKSTATION, Action.MANAGE, NEVER_END),
            conn,
        )
        await CapabilitiesRepository.add(
            person_id,
            response.id,
            CapabilityInput(Resource.WORKSTATION, Action.READ, NEVER_END),
            conn,
        )
        logger.info(
            "workstation.created",
            extra={
                ...
                "workstation_id": str(response.id),
    ...
    async def update(
        ...
        await WorkstationValidator.validate_manage_capability(
            person_id, workstation_id, conn
        )
        r = await WorkstationRepository.update(
            workstation_id, RequestTranslator.update_request_to_changes(request), conn
        )
        if not r:
            raise WorkstationNotFoundError
        logger.info(
    ...
    async def get(
        ...
        await WorkstationValidator.validate_read_capability(
            person_id, workstation_id, conn
        )
