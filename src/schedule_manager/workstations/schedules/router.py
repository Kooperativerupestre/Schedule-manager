...
async def add_workstation_schedule(request:ScheduleAddRequest, person_id:UUID = Depends(get_current_person_id), conn: AsyncConnection[DictRow] = Depends(get_transaction)):
    return await ScheduleService.add(person_id, request, conn)

@router.delete('/{workstation_id}/{schedule_id}')
async def delete_workstation_schedule(workstation_id:UUID,
                                schedule_id:UUID,
...
async def update_workstation_schedule(workstation_id:UUID,
                                schedule_id:UUID,
                                request:ScheduleChangesRequest,
                                person_id:UUID = Depends(get_current_person_id),
                                conn: AsyncConnection[DictRow] = Depends(get_transaction)):
    return await ScheduleService.update(person_id, workstation_id, schedule_id, request, conn)
@router.get("/{workstation_id}/{schedule_id}")
async def get_workstation_schedule(workstation_id:UUID,
                                schedule_id:UUID,
                                person_id:UUID = Depends(get_current_person_id),
...
async def can_schedule(
    ...
    person_id:UUID = Depends(get_current_person_id), 
    conn: AsyncConnection[DictRow] = Depends(get_connection)):
    return await ScheduleService.can_schedule(person_id, business_id, unit_id, workstation_id, validity_range, conn)
