...
from schedule_manager.workstations.holidays.repository import (
    HolidayConfigWorkstationHolidays as HolidayRepositoryContext,
)
...
class WorkstationHolidayService:
    ...
    async def add(
        ...
        await CapabilitiesValidator.validate_manage_capability(
            person_id, Resource.WORKSTATION_HOLIDAYS, business_id, conn
        )

        try:
            id = await HolidayRepository.add(
                HolidayRepositoryContext,
                business_id,
                RequestTranslator.add_request_to_holiday(request),
                conn,
            )
            logger.info(
                "workstation_holiday.created",
                extra={
                    "actor_id": str(person_id),
                    "workstation_id": str(business_id),
                    "holiday_id": str(id),
    ...
    async def delete(
        ...
        has_deleted = await HolidayRepository.delete(
            HolidayRepositoryContext, holiday_id, conn
        )

        if not has_deleted:
            raise NotFoundError
        logger.info(
            "workstation_holiday.deleted",
            extra={
                "actor_id": str(person_id),
                "workstation_id": str(business_id),
    ...
    async def update(
        ...
        has_updated = await HolidayRepository.update(
            HolidayRepositoryContext,
            holiday_id,
            RequestTranslator.update_request_to_holiday_changes(changes),
            conn,
        )
        if not has_updated:
            raise NotFoundError
        await conn.commit()
        logger.info(
            "workstation_holiday.updated",
            extra={
                "actor_id": str(person_id),
                "workstation_id": str(business_id),
                "holiday_id": str(holiday_id),
                "request": model_context(changes),
            },
        )
    ...
    async def get(
        ...
        await CapabilitiesValidator.validate_read_capability(
            person_id, Resource.WORKSTATION_HOLIDAYS, workstation_id, conn
        )
    ...
    async def has_overlapping_interval(
        ...
        return await HolidayRepository.has_overlapping_interval(
            HolidayRepositoryContext,
            workstation_id,
            RequestTranslator.holiday_range_request_to_holiday_range(request),
            conn,
        )
