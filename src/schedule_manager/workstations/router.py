...
@router.post("")
async def add(request: WorkstationAddRequest, person_id: UUID = Depends(get_current_person_id), conn: AsyncConnection[DictRow] = Depends(get_transaction)):
    return await WorkstationService.add(person_id, request, conn)


@router.delete("/{unit_id}")
async def delete(unit_id: UUID, workstation_id: UUID, person_id: UUID = Depends(get_current_person_id), conn: AsyncConnection[DictRow] = Depends(get_transaction)):
    await WorkstationService.delete(person_id, unit_id, workstation_id, conn)


@router.patch("/{workstation_id}")
async def update(workstation_id: UUID, request: WorkstationUpdateRequest, person_id: UUID = Depends(get_current_person_id), conn: AsyncConnection[DictRow] = Depends(get_transaction)):
    await WorkstationService.update(person_id, workstation_id, request, conn)


@router.get("/{workstation_id}")
async def get(workstation_id: UUID, person_id: UUID = Depends(get_current_person_id), conn: AsyncConnection[DictRow] = Depends(get_connection)):
