from schedule_manager.auth.models import (
    Authentication,
    AuthenticationRow,
    Providers,
...
def authentication_row_to_model_authentication(
...
def authentication_row_to_model_authentication_output(
    row: AuthenticationRow,
) -> AuthenticationOutput:
...
"""
        values = (
            person_id,
            authentication_data.provider.value,
            person_id,
            authentication_data.db_credentials,
        )
        try:
            ...
            log_repository_error(
                AuthenticationRepository, "add", e, {"person_id": str(person_id)}
            )
...
"""
    ...
    async def get_all_from_person(
        conn: AsyncConnection[DictRow], person_id: UUID
    ) -> list[AuthenticationOutput]:
        async with conn.cursor(row_factory=class_row(AuthenticationRow)) as cur:
            await cur.execute(
                "SELECT person_id, provider, identifier, credentials FROM authentication WHERE person_id = %s",
                (person_id,),
            )
            result = await cur.fetchall()

            return [
                authentication_row_to_model_authentication_output(r) for r in result
            ]

    @staticmethod
    async def get_from_person_with_provider(
        conn: AsyncConnection[DictRow], person_id: UUID, provider: Providers
    ) -> AuthenticationOutput | None:
        query = """
...
"""
    ...
    @staticmethod
    async def get(
        conn: AsyncConnection[DictRow], id: UUID
    ) -> list[AuthenticationOutput]:
