...
from schedule_manager.units.models import (
    Unit,
    UnitRow,
    UnitAddOutput,
    UnitGetOutput,
...
"""
        ...
        try:
            ...
            log_repository_error(
                UnitRepository, "add", error, {"business_id": str(unit.business_id)}
            )
            raise ValueError(f"Business with id {unit.business_id} does not exist")
        return UnitAddOutput(id=r.id, created_at=r.created_at)
...
"""
    ...
    @staticmethod
    async def update(
        id: UUID, changes: UnitChanges, conn: AsyncConnection[DictRow]
    ) -> bool:
        updates = []
        values = []
