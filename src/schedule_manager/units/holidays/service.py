...
from schedule_manager.units.holidays.repository import (
    HolidayConfigUnitHolidays as HolidayRepositoryContext,
)
...
class UnitHolidayService:
    ...
    async def add(
        ...
        await CapabilitiesValidator.validate_manage_capability(
            person_id, Resource.UNIT_HOLIDAYS, unit_id, conn
        )
    ...
    @staticmethod
    async def delete(
        person_id: UUID, unit_id: UUID, holiday_id: UUID, conn: AsyncConnection[DictRow]
    ) -> None:
        await CapabilitiesValidator.validate_manage_capability(
            person_id, Resource.UNIT_HOLIDAYS, unit_id, conn
        )
    ...
    async def has_overlapping_interval(
        ...
        return await HolidayRepository.has_overlapping_interval(
            HolidayRepositoryContext,
            unit_id,
            RequestTranslator.holiday_range_request_to_holiday_range(validity_range),
            conn,
        )
