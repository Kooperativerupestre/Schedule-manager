...
from schedule_manager.people.schemas import PersonUpdateRequest, LocalPersonCreateRequest
...
@router.post('/local')
async def add_local(data:LocalPersonCreateRequest, response:Response, conn:AsyncConnection[DictRow]=Depends(get_transaction)):
    try:
...
@router.delete('/')
async def delete(id:UUID = Depends(get_current_person_id), conn:AsyncConnection[DictRow] = Depends(get_transaction)):
    try:
...
@router.patch('/')
async def update(update:PersonUpdateRequest, id:UUID = Depends(get_current_person_id), conn:AsyncConnection[DictRow] = Depends(get_transaction)):
    try:
        ...
        raise HTTPException(status_code=500)
@router.get('/')
async def get(id:UUID = Depends(get_current_person_id), conn:AsyncConnection[DictRow] = Depends(get_connection)):
    try:
