...
class PeopleRepository:
    @staticmethod
    async def add(
        conn: AsyncConnection[DictRow], person: AddPersonInput
    ) -> AddPersonOutput:
        try:
            ...
            log_repository_error(
                PeopleRepository, "add", error, {"phone_number": person.phone_number}
            )
            raise PhoneNumberAlreadyExistsError(
                "Phone number {} already exists".format(person.phone_number)
            )
    ...
    async def get(conn: AsyncConnection[DictRow], id: UUID) -> GetPerson | None:
        async with conn.cursor(row_factory=class_row(GetPerson)) as cur:
            await cur.execute(
                "SELECT name, phone_number, id, created_at, status FROM people WHERE id = %s",
                (id,),
            )
            result = await cur.fetchone()
            return result
    ...
    async def get_latest(conn: AsyncConnection[DictRow], n: int) -> list[GetPerson]:
        async with conn.cursor(row_factory=class_row(GetPerson)) as cur:
            await cur.execute(
                "SELECT name, phone_number, id, created_at, status FROM people ORDER BY created_at DESC LIMIT %s",
                (n,),
            )
    ...
    async def delete(conn: AsyncConnection[DictRow], id: UUID) -> GetPerson | None:
        async with conn.cursor(row_factory=class_row(GetPerson)) as cur:
            await cur.execute(
                "DELETE FROM people WHERE id = %s RETURNING name, phone_number, id, created_at, status",
                (id,),
            )
    ...
    @staticmethod
    async def update(
        conn: AsyncConnection[DictRow], id: UUID, update: UpdatePerson
    ) -> bool:
        updates = []
        ...
        try:
            ...
            log_repository_error(
                PeopleRepository, "update", error, {"person_id": str(id)}
            )
            raise PhoneNumberAlreadyExistsError(
