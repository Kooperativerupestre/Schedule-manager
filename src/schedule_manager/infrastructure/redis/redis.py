...
LUA_SCRIPT_PATH = Path(__file__).with_name("lua_script.lua")
...
async def lifespan(app: FastAPI):
    ...
    lua_script = Path(__file__).with_name("lua_script.lua").read_text(encoding="utf-8")

    script = redis.register_script(lua_script)

    app.state.redis = redis
    app.state.rate_limit_script = script
