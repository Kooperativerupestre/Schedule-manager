...
from schedule_manager.business.memberships.repository import (
...
class MembershipService:
    ...
    async def add(
        ...
        await MembershipValidator.validate_capability_membership(
            person_id, business_id, Action.MANAGE, conn
        )
        try:
    ...
    async def has(
        ...
        if person_id == target_person_id:
            return await MembershipRepository.has(
                target_person_id, business_id, conn, status
            )

        await MembershipValidator.validate_capability_membership(
            person_id, business_id, Action.READ, conn
        )
        return await MembershipRepository.has(
            target_person_id, business_id, conn, status
        )
    ...
    async def get(
        ...
        if person_id == target_person_id:
            return await MembershipRepository.get(
                target_person_id, business_id, conn, status
            )
        await MembershipValidator.validate_capability_membership(
            person_id, business_id, Action.READ, conn
...
class MembershipInviteService:
    ...
    async def add(
        ...
        await MembershipValidator.validate_capability_membership(
            person_id, business_id, Action.INVITE, conn
        )
        id = await MembershipInvitesRepository.add(
            business_id,
            BusinessMembershipInviteInput(request.email, request.expires_at),
            conn,
        )
        logger.info(
            "membership_invite.created",
            extra={
                "actor_id": str(person_id),
                "business_id": str(business_id),
                "invite_id": str(id),
                "request": model_context(request.email),
            },
        )
        return id
    ...
    async def end(
        ...
        await CapabilitiesRepository.end_all_from_target(
            person_id, invite_id, Scope.BUSINESS, conn
        )
        r = await MembershipInvitesRepository.end(invite_id, conn)
        if not r:
            raise MembershipInviteNotFoundError
        logger.info(
            "membership_invite.ended",
            extra={
                "actor_id": str(person_id),
                "business_id": str(business_id),
