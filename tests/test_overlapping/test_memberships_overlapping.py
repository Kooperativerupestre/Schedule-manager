...
async def test_same_membership_interval_only_allows_one_insert(
    ...
    owner = await person_factory(
        "membership-owner", "membership-owner-phone", setup_conn
    )
    member = await person_factory(
        "membership-member", "membership-member-phone", setup_conn
    )
    ...
    results = await asyncio.gather(
        *(insert_membership(connection) for connection in connections),
...
async def test_same_membership_invite_interval_only_allows_one_insert(
    ...
    async def insert_invite(connection) -> None:
        async with connection.transaction():
            await MembershipInvitesRepository.add(
                business_id,
                BusinessMembershipInviteInput("overlap@example.com", end),
                connection,
            )

    results = await asyncio.gather(
        *(insert_invite(connection) for connection in connections),
        return_exceptions=True,
    )
    assert results.count(None) == 1
    assert all(
