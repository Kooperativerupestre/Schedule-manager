...
async def test_add_member_forbidden(
    conn: AsyncConnection[DictRow], single_person, business
) -> None:
    with pytest.raises(ForbiddenError):
        await MembershipService.add(single_person.id, single_person.id, business, conn)
...
async def test_end_member_forbidden(
    ...
    with pytest.raises(ForbiddenError):
        await MembershipService.end(single_person.id, single_person.id, business, conn)


async def test_get_member_forbidden(
    conn: AsyncConnection[DictRow], person_factory, business
) -> None:
    p_1 = await person_factory("3", "3")
    p_2 = await person_factory("4", "4")
    with pytest.raises(ForbiddenError):
        await MembershipService.get(p_1.id, p_2.id, business, conn)
...
async def test_has_member_forbidden(
    ...
    p_2 = await person_factory("4", "4")
    with pytest.raises(ForbiddenError):
        await MembershipService.has(p_1.id, p_2.id, business, None, conn)
...
async def test_add_member_allowed(
    conn: AsyncConnection[DictRow], create_business_with_owner, person_factory
) -> None:
...
async def test_end_member_allowed(
    ...
    member = await person_factory("4", "4")
    await MembershipService.add(owner.id, member.id, business_id, conn)
    await MembershipService.end(owner.id, member.id, business_id, conn)
...
async def test_get_member_allowed(
    ...
    member = await person_factory("4", "4")
    await MembershipService.add(owner.id, member.id, business_id, conn)
    await MembershipService.get(owner.id, member.id, business_id, conn)
...
async def test_has_member_allowed(
    ...
    member = await person_factory("4", "4")
    await MembershipService.add(owner.id, member.id, business_id, conn)
    await MembershipService.has(owner.id, member.id, business_id, None, conn)
